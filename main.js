...
//   "use strict";
  ...
  var RESOLUTION = 1/1024; // The smaller, the more accurate the solution. Try 1/1024 if Floating point errors arise.
  var FRAMERATE = 60;
  ...
  var fact = function (n) {
    // maybe sanitise input so we don't run into non-natural n
    var prod = 1;
    for (let i = 2; i <= n; i++) {
      prod *= i;
    };
...
var options = {
  xEquation: "0.2 * y[0] + x[0] - 0.1 * x[1]",
  xOrder: 2,
  xDerivatives: [0,3],
  yEquation: "1 - x[0] + x[1] - y[0] - t",
  yOrder: 1,
  yDerivatives: [2]
};
...
function loop (delta) {
    ...
    [circle.x, circle.y] = cartesianToGrid(...particles[0].solution.tick(t));
...
var setup = function () {
  ...
  particles[0].bindSystem(equationSystem);

  // implement


  particles[0].solution = particles[0].solveEuler();
